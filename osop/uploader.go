...
func UploadMultiPartFromReader(svc *s3.S3, reader io.Reader, bucket, key string) error {
	...
	if err != nil {
		err = fmt.Errorf("startUploadPart failed:%v", err)
		return err
	}
	if _, err := putParts(svc, reader, bucket, key, uploadId); err != nil {
		err = fmt.Errorf("putParts failed:%v", err)
		if err2 := abortPutPart(svc, bucket, key, uploadId); err2 != nil {
			err = fmt.Errorf("%v,abortPutPart failed:%v", err, err2)
		}
		return err
	}
